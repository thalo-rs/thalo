use awto_es::{postgres::tls::NoTls, Error, InternalError};
use bb8_postgres::{
...
impl BankAccountViewRepository {
    ...
    pub async fn load(&self, id: &str) -> Result<Option<(BankAccountView, i64)>, Error> {
        let conn = self.get_conn().await?;
    ...
    pub async fn last_event_id(&self) -> Result<Option<i64>, Error> {
        let conn = self.get_conn().await?;
